...
extern void *disp_malloc(u32 num_bytes, u32 *phy_addr);
...
struct eink_buffer_manager {
	...
	s32 (*queue_image)(struct eink_buffer_manager* buffer_mgr, void * src_image, unsigned int mode,  struct area_info update_area);
...
struct pipeline_manager {
	...
	int (*used_list_status)(struct pipeline_manager *);
	int (*free_list_status)(struct pipeline_manager *);
...
struct wavedata_queue {
	...
	unsigned int 				wavedata_paddr[WAVE_DATA_BUF_NUM];
...
struct eink_private {
	...
	bool						enable_flag;
	unsigned int 				index_paddr[INDEX_BUFFER_NUM];
